...
Условий может быть сколько угодно много, для этого используеться конструкция `elif`, которая также как `if` может проверять условия:
```sh
read COMMAND
...
then
	...
	echo "Команда не определена. Воспользуйетсь командой 'help' для справки"
fi
```
...
| -n       | проверка есть ли в строке хоть один символ  | if [ -z $str ]           |