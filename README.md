...
-   ### Что такое DNS
    ...
    -   [IP-адрес](https://ru.wikipedia.org/wiki/IP-%D0%B0%D0%B4%D1%80%D0%B5%D1%81)
        > Специальный номерной идентификатор предоставляемый устройству находящемуся в сети работающей по [протоколу IP](#cтек-протоколов-tcpip)
...
-   ### WebSockets
    ...
    [Веб-сокеты](https://ru.wikipedia.org/wiki/WebSocket) это продвинутая технология, позволяющая открыть постоянное двунаправленное сетевое соединение между браузером пользователя и сервером. С помощью его API вы можете отправить сообщение на сервер и получить ответ без выполнения http запроса, тем самым реализуя real-time взаимодействие. 

    Основная идея в том, что вам ненужно посылать запросы на сервер для получения новой информации. Когда соединение установлено сервер сам отправит новую порцию данных подключенным клиентам. Веб-сокеты широко используются для создания чатов, онлайн-игр, трейдерских приложений и т.д.
